...
public class Q1 {
    ...
    public static double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        // 分割线左边的所有元素需要满足的个数 m + (n - m + 1) / 2 -> (m + n + 1) / 2;
        // 两个数组长度之和为偶数时，当在长度之和上+1时，由于整除是向下取整，所以不会改变结果
        // 两个数组长度之和为奇数时，按照分割线的左边比右边多一个元素的要求，此时在长度之和上+1，就会被2整除，会在原来的数的基础上+1
        ...
        // nums1[i - 1] <= nums2[j]
        //  此处要求第一个数组中分割线的左边的值 不大于(等于) 第二个数组中分割线的右边的值
        // nums2[j - 1] <= nums1[i]
        //  此处要求第二个数组中分割线的左边的值 不大于(等于) 第一个数组中分割线的右边的值
        ...
        while (left < right) {
            ...
            // 第一个数组中左边的元素个数确定后，用 左边元素的总和 - 第一个数组中左边元素的总和 = 第二个元素中左边的元素的总和
            //                                   -> 左边元素的总和 = 第一个数组中左边元素的总和 + 第二个元素中左边的元素的总和
            // 此时j就是第二个元素中左边的元素的个数
            int j = totalLeft - i;
